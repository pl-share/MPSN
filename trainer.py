...
#rom src.vis_tool import Visualizer
...
class Head_Detector_Trainer(nn.Module):
    ...
    def forward(self, imgs,imgs2, bboxs, scale):
        n,_,_ = bboxs.size()
        if n != 1:
        ...
        #h2 = self.head_detector.left_add(x2)
        h3=self.head_detector.vgg(x)

        #hf1 = h1 + h2

        #h3 = self.head_detector.extractor(hf1)
        h4 = self.head_detector.addnet(h2)
        #hf2=h3+h4
